...
static pthread_key_t g_pThreadEnvKey;
...
void detachCurrentThread(void *pEnv) {
    assert(g_cachedJVM);
    g_cachedJVM->DetachCurrentThread();
}

void createThreadEnvKey() {
    (void) pthread_key_create(&g_pThreadEnvKey, detachCurrentThread);
}
...
JNIEnv * jniGetThreadEnv() {
    ...
    if(get_res == JNI_EDETACHED){
        ...
        pthread_setspecific(g_pThreadEnvKey, env);
    }
